...
class JoyControl(object):
  ...
  def receiveJoy(self, joy = None):
    limits = self.getLimits()

    servos = range(8)
    frequency = [1.25]*len(servos)
    frequency[6] = 1.5
    ...
    if button >= 0 and button < 4:
      ...
      amplitude[1] = joy.axes[1]*left_gain*20.0*math.pi/180.0
      amplitude[3] = -joy.axes[1]*right_gain*20.0*math.pi/180.0
      ...
      # back flap
      amplitude[6] = joy.axes[1]*15.0*math.pi/180.0
      offset[6] = joy.axes[3]*20.0*math.pi/180.0
